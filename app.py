...
.header-bar {{ background:#1A2E44; padding:8px; color:#FDFCF0; font-family:serif; text-align:center; letter-spacing:1px; font-size:1.1rem; }}
.art-box {{ background:white; padding:25px; border-radius:5px; box-shadow:0 2px 8px rgba(0,0,0,0.05); margin-bottom:20px; width:100%; }}
@media (max-width:768px) {{ .header-image {{ height:60px; }} .art-box {{ padding:15px; }} .header-bar {{ font-size:0.9rem; }} }}
.i-title {{ display:flex; align-items:center; gap:8px; margin-top:15px; font-family:serif; font-weight:bold; color:#1A2E44; font-size:1rem; }}
...
with c1:
    ...
    if cur:
        st.markdown(f"""<div class="art-box">
            <h1 style="font-family:serif; color:#1A2E44; font-size:1.8rem; margin-bottom:5px;">{cur["titolo"]}</h1>
            <p style="color:#C5A059; font-style:italic; margin-bottom:20px;">{cur["data"]}</p>
            <div style="font-family:serif; font-size:1.15rem; line-height:1.7; color:#1A2E44;">
                {cur["testo"].replace("\\n","<br>")}
            </div>
        </div>""", unsafe_allow_html=True)
    else:
        st.info("Benvenuti su Sorgente Yoga. Seleziona un articolo dalla biblioteca.")

    # --- AREA AMMINISTRATORE (IN FONDO SU MOBILE) ---
    st.divider()
    if not st.session_state['adm']:
        ...
        st.success("✍️ MODALITÀ EDITORE ATTIVA")
        ...
        with st.expander("✏️ MODIFICA / ELIMINA"):
            if all_a:
                ...
                all_a[idx]['cat'] = st.radio("Sezione", ["BLOG", "TESTI"], index=0 if all_a[idx].get('cat','BLOG')=="BLOG" else 1, key="e_cat")
                all_a[idx]['titolo'] = st.text_input("Titolo", all_a[idx]['titolo'], key="e_tit")
                all_a[idx]['testo'] = st.text_area("Contenuto", all_a[idx]['testo'], height=500, key="e_tx")
...
with c2:
    ...
    with st.expander("Articoli Blog", expanded=True):
