...
if 'mode' not in st.session_state: st.session_state['mode'] = "view" # view, edit, new
...
if st.session_state['mode'] == "view" and idx is not None:
    display = all_a[idx]
    ...
    new_c = st.radio("Sezione", ["BLOG", "TESTI"], index=0 if curr['cat']=="BLOG" else 1)
...
ce1, ce2, ce3 = st.columns([0.3, 0.3, 0.4])

with ce1:
    if st.button("➕ NUOVO ARTICOLO"):
        st.session_state['mode'] = "new"
        st.rerun()
with ce2:
    if st.button("📝 MODIFICA CORRENTE"):
